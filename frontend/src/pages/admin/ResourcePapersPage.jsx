...
import {
    ...
    uploadCoverImage,
    uploadPDF
} from '@/services/resourcePapersService';

const ResourcePapersPage = () => {
    ...
    const { data: papers, isLoading } = useQuery({
        queryKey: ['resource-papers'],
        queryFn: getResourcePapers
    });

    const createMutation = useMutation({
        mutationFn: createResourcePaper,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['resource-papers'] });
    ...
    const updateMutation = useMutation({
        mutationFn: ({ id, data }) => updateResourcePaper(id, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['resource-papers'] });
    ...
    const deleteMutation = useMutation({
        mutationFn: deleteResourcePaper,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['resource-papers'] });
        ...
        <AdminLayout>
            <div className="max-w-7xl mx-auto">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Resource Papers</h1>
                        <p className="text-gray-600 dark:text-gray-400 mt-1">Manage your research papers and publications</p>
                    </div>
                    <button
                ...
                >
                    <form onSubmit={handleSubmit}>
                        <FormField label="Title" name="title" value={formData.title} onChange={handleInputChange} required placeholder="Paper Title" />
                        <FormField
                            label="Slug (optional)"
                            name="slug"
                            value={formData.slug}
                        ...
                        <FormField
                            ...
                            onChange={handleInputChange}
                            rows={5}
                            placeholder="Full paper content or abstract..."
                        />
                        <FormField
                            label="External URL"
                            name="external_url"
                        ...
                        <div className="flex items-center gap-2 mb-4">
                            <input
                                type="checkbox"
                                name="is_featured"
                                id="paper_is_featured"
                                checked={formData.is_featured}
                                onChange={handleInputChange}
                                className="rounded"
                            />
                            <label htmlFor="paper_is_featured" className="text-sm text-gray-700 dark:text-gray-300">
                                Feature this paper
                            </label>
                        </div>
...
export default ResourcePapersPage;
